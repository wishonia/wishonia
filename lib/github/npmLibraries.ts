...
export async function getOwnerRepoFromNpm(libraryName: string): Promise<string | null | false> {
...
function extractRepoFromPackageData(
    data: { name: string, repository?: { url: string }; homepage?: string; bugs?: { url: string }} ) {
    ...
    if (repositoryUrl) {
        let match = repositoryUrl.match(/https?:\/\/(?:www\.)?github\.com\/([^/]+\/[^/]+)/i);
        if(!match){
...
async function getReposFromPackageJson(packageJsonPath: string | undefined) {
    ...
    console.log('Could not find repositories for the following libraries:', couldNotFind.join('\n, '));
    let validOwnerRepoArr = ownerRepoArr.filter((path): path is string => path !== null);
    return Array.from(new Set(validOwnerRepoArr)); // Remove duplicates
}

export async function scoreContributorsFromPackageJson(packageJsonPath?: string, topN: number = 10) {
