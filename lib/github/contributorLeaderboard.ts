...
async function getContributors(owner: string, repo: string) {
    ...
    while (true) {
        const response = await octokit.repos.listContributors({owner, repo, per_page: 100, page});
...
export async function contributorLeaderboard(ownerRepoArr: string[]): Promise<void> {
