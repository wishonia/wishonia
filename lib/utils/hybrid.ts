...
export interface WishoniaLibArgs extends BaseRetrieverInput {
  agentId: string;
...
export class HybridRetrival extends BaseRetriever {
  static lc_name() {
    return "HybridRetrival";
  }

  lc_namespace = ["langchain", "retrievers", "wishonia"];
  agentId: string;
  sourceId: string | null;
  ...
  protected async similaritySearch(
    ...
    try {
      ...
      const bot_id = this.agentId;

      const data = await prisma.$queryRaw`
     SELECT * FROM "similarity_search_v2"(query_embedding := ${vector}::vector, agentId := ${bot_id}::text,match_count := ${k}::int)
    `;
  ...
  protected async keywordSearch(
    ...
    const botInfo = await prisma.agent.findFirst({
      where: {
        id: bot_id,
      },
    });
