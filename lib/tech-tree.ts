...
const trees = {
    longevity: 'https://raw.githubusercontent.com/foresight-org/LongevityTechTree/main/ForesightTechTree/longevity.graph',
    neurotech: 'https://raw.githubusercontent.com/foresight-org/LongevityTechTree/main/ForesightTechTree/neurotech.graph',
    cooperation: 'https://github.com/foresight-org/LongevityTechTree/blob/main/ForesightIntcoopTree/intcoop.graph'
}
export async function getTechTree(type: 'longevity' | 'neurotech' | 'cooperation'){
...
export async function getArrayOfLabelsForEachParent(type: 'longevity' | 'neurotech' | 'cooperation') {
...
export async function getSolutionsFromTechTree(type: 'longevity' | 'neurotech' | 'cooperation'){
    const techTree = await getTechTree(type);
...
export async function saveCleanedTechTree(type: 'longevity' | 'neurotech' | 'cooperation'){
    const techTree = await getTechTree(type);
    return fs.writeFileSync(absPathFromRepo(
        `public/data/${type}-tech-tree-cleaned.json`),
...
export async function getClassifiedTechTreeObjects(
    ...
    for (const item of techTree) {
        ...
        const openAIClassification =
            await classifyText(item.label, classificationOptions, `The type as it pertains to ${type}`);
...
function saveTechTreeClassification(type: 'longevity' | 'neurotech' | 'cooperation', classification: any) {
    fs.writeFileSync(getPathToTechTreeClassification(type), JSON.stringify(classification, null, 4));
}

function getPathToTechTreeClassification(type: 'longevity' | 'neurotech' | 'cooperation') {
    return absPathFromRepo(`public/data/${type}-tech-tree-classified.json`);
}

function alreadyClassifiedTechTree(label: string, type: 'longevity' | 'neurotech' | 'cooperation') {
    const classified = getTechTreeClassification(type);
...
function getTechTreeClassification(type: 'longevity' | 'neurotech' | 'cooperation'): { [key: string]: any[] } {
    ...
    return JSON.parse(fs.readFileSync(getPathToTechTreeClassification(type)).toString());
}
    ...
    for (const item of solutionItems) {
        ...
        if(globalSolution){
            ...
            if(existingGlobalProblemSolution) {
                console.log(`Skipping solution "${solutionName}" because it already exists`);
                continue;
            }
        }
        const goodSolution = await isGoodSolution(globalProblem.name, solutionName);
        if(!goodSolution){
            console.log(`Skipping solution "${solutionName}" because it was not a good solution for ${globalProblem.name}`);
            continue;
        }
        let solutionDescription = item.desc || null;
        const newGlobalProblemSolution =
            await createGlobalProblemSolution(solutionName, solutionDescription, globalProblem);
