...
function getOutputFilePath(filePath: string, pythonProjectDirectory: string, outputDirectory: string) {
    let outputFilePath = filePath.replace('.py', '.ts')
    return outputFilePath.replace(pythonProjectDirectory, outputDirectory)
}
const convertPythonToGithubIssue = async (pythonCode: string): Promise<{ title: string, body: string }> => {
...
export async function convertPythonCodeToGithubIssues(pythonProjectDirectory: string,
    ...
    for (const file of pythonFiles) {
        ...
        let filePath = getOutputFilePath(file, pythonProjectDirectory, outputDirectory);
...
export const convertPythonProjectToNextJS = async (
    ...
    for (const file of pythonFiles) {
        ...
        let outputFilePath = getOutputFilePath(file, pythonProjectDirectory, outputDirectory);
...
function getCodeFromBlocks(result: string) {
    const matchTypescript =
        result.match(/```typescript\n([\s\S]*?)\n```/);
    const matchTsx =
        result.match(/```tsx\n([\s\S]*?)\n```/);
