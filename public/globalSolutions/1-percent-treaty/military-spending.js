...
function calculateLinearRegression(years, spending) {
  ...
  let sum_xy = years
    .map((year, idx) => year * spending[idx])
    .reduce((a, b) => a + b, 0)
...
let spendingProvided = [
  ...
  0.81, 0.82, 0.83, 0.84, 0.85, 0.86, 0.87, 0.88, 0.9, 0.92, 0.94, 0.96, 1.0,
  1.03, 1.09, 1.14, 1.21, 1.28, 1.34, 1.39, 1.42, 1.47, 1.54, 1.57, 1.62, 1.63,
  1.62, 1.63, 1.64, 1.66, 1.69, 1.74, 1.82, 1.91, 1.92, 1.98,
]
...
let spending2023 = linearModel(
  2023,
  ...calculateLinearRegression(yearsProvided, spendingProvided)
)
let projectedFilteredSpending = createProjectedData(
  yearsProvided,
  spendingProvided,
  2023,
  projectionEndYear
)
let spendingReduction = calculateSpendingReduction(
  spending2023,
  2023,
  projectionEndYear,
  0.01
...
let adjustedLabels = yearsProvided.concat(
  Array.from({ length: projectionEndYear - 2021 }, (_, i) => i + 2022)
)

// Adjust projectedFilteredSpending to start from 2023
let adjustedProjectedFilteredSpending = Array(
  lastProvidedYear - yearsProvided[0] + 1
)
  .fill(null)
  .concat(projectedFilteredSpending.slice(lastProvidedYear - startYear + 1))

// Adjust spendingReduction to start from 2023
let adjustedSpendingReduction = Array(lastProvidedYear - yearsProvided[0] + 1)
  .fill(null)
  .concat(spendingReduction)

// Adjust spendingFreeze to start from 2023
let adjustedSpendingFreeze = Array(lastProvidedYear - yearsProvided[0] + 1)
  .fill(null)
  .concat(spendingFreeze)
