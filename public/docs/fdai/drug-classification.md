...
OTC Score = (ER * 0.3) + ((100 - AER) * 0.2) + ((100 - SAER) * 0.2) + ((10 - DII) * 10 * 0.1) + (SAS * 10 * 0.2)

If OTC Score ≥ 80, consider for OTC status

## Prescription-Only Determination

Prescription Score = (ER * 0.25) + (AER * 0.15) + (SAER * 0.2) + (DII * 5 * 0.1) + ((10 - SAS) * 10 * 0.3)

If Prescription Score ≥ 60, consider for prescription-only status

## DEA Scheduling Determination

Scheduling Score = (APS * 0.3) + (WSI * 10 * 0.2) + (ORF * 10 * 0.2) + (MP * 0.2) + ((10 - TI) * 10 * 0.1)
...
Access Restriction Benefit Score (ARBS) = (100 - ABR) * 0.6 + (100 - SCI) * 0.4
...
(OTC Score * 0.3) + (Prescription Score * 0.3) + (Scheduling Score * 0.2) + (ARBS * 0.2)
