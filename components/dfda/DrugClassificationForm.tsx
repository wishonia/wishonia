...
const fieldDescriptions: Record<keyof DrugClassificationProfile, string> = {
  ...
  therapeuticIndex:
    "Safety margin between effective and toxic dose (higher is safer)",
  ...
  selfAdministrationSafety:
    "How safely patients can use the drug on their own (0-10)",
  accessBenefitRatio:
    "Benefits vs. risks of easy access (higher means more benefit)",
  societalCostIndex:
    "Social and economic costs of restricting the drug (0-100)",
  ...
  accessRestrictionBenefitScore:
    "Score evaluating the benefits of access restrictions",
  finalClassificationScore:
    "Final score used to determine the drug classification",
...
const classificationDefinitions: Record<Classification, string> = {
  ...
  SCHEDULE_II:
    "Schedule II: High abuse risk, severe psychological or physical dependence",
  SCHEDULE_III:
  ...
  SCHEDULE_V:
    "Schedule V: Very low potential for abuse, limited quantities of certain narcotics",
...
const DrugClassificationForm: React.FC = () => {
  ...
  const [activeTooltip, setActiveTooltip] = useState<
    keyof DrugClassificationProfile | null
  >(null)

  const handleInputChange = (
    name: keyof DrugClassificationProfile,
    value: number | string
  ) => {
  ...
  useEffect(() => {
    const calculateScores = (
      data: DrugClassificationProfile
    ): Partial<DrugClassificationProfile> => {
      const otcScore =
        data.efficacyRate * 0.3 +
      ...
      else if (prescriptionScore > otcScore)
        classification = "PRESCRIPTION_ONLY"
    ...
    formData.efficacyRate,
    formData.adverseEventRate,
    formData.severeAdverseEventRate,
    formData.drugInteractionIndex,
    formData.selfAdministrationSafety,
    formData.abusePotentialScore,
    formData.withdrawalSeverityIndex,
    formData.overdoseRiskFactor,
    formData.misuseProbability,
    formData.therapeuticIndex,
    formData.accessBenefitRatio,
    formData.societalCostIndex,
  ])
      ...
      default:
        return (
          <input
            ...
            value={value}
            onChange={(e) => handleInputChange(key, e.target.value)}
            className="w-24 rounded border border-black bg-gray-100 p-1 text-right"
            step="0.01"
          />
        )
    }
  ...
  const chartData = [
    ...
    { name: "Scheduling", score: formData.schedulingScore },
    {
      ...
      score: formData.accessRestrictionBenefitScore,
    },
    { name: "Final", score: formData.finalClassificationScore },
  ]

  return (
    <div className="mx-auto w-full max-w-4xl bg-gradient-to-b from-blue-100 to-purple-100 p-8 font-sans">
      <h2 className="mb-8 rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 p-4 text-center text-4xl font-bold text-white shadow-lg">
        DRUG CLASSIFICATION SIMULATOR
      </h2>
      <div className="mb-8 rounded-lg bg-white p-6 shadow-lg">
        <h3 className="mb-4 rounded bg-gradient-to-r from-blue-500 to-purple-500 p-2 text-2xl font-bold text-white">
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center">
                    {renderInput(key)}
                    <span className="ml-2 w-16 rounded bg-gray-200 p-1 text-right">
      ...
      <div className="mb-8 rounded-lg bg-white p-6 shadow-lg">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <div
            className={`rounded-lg border-4 p-4 ${formData.classification === "OTC" ? "border-green-500 bg-green-100" : "border-gray-300 bg-gray-100"}`}
          >
          ...
          <div
            className={`rounded-lg border-4 p-4 ${formData.classification === "PRESCRIPTION_ONLY" ? "border-yellow-500 bg-yellow-100" : "border-gray-300 bg-gray-100"}`}
          >
            <h4 className="flex items-center font-bold">
          ...
          <div
            className={`rounded-lg border-4 p-4 ${["SCHEDULE_I", "SCHEDULE_II", "SCHEDULE_III", "SCHEDULE_IV", "SCHEDULE_V"].includes(formData.classification || "") ? "border-red-500 bg-red-100" : "border-gray-300 bg-gray-100"}`}
          >
            <h4 className="flex items-center font-bold">
              {[
                "SCHEDULE_I",
                ...
                "SCHEDULE_IV",
                "SCHEDULE_V",
              ].includes(formData.classification || "") ? (
        ...
        <div className="mt-4 rounded-lg border-2 border-gray-300 bg-gray-100 p-4">
          <h4 className="font-bold">CLASSIFICATION DEFINITION:</h4>
          <p className="mt-2 text-sm">
            {
              classificationDefinitions[
                formData.classification || "UNSCHEDULED"
              ]
            }
          </p>
        </div>
      ...
      <div className="rounded-lg bg-white p-6 shadow-lg">
        ...
        <div className="mt-4 space-y-4">
          <div>
            ...
            <InlineMath
              math={`${formData.otcScore?.toFixed(2)} = (\\text{Efficacy Rate: }${formData.efficacyRate} * 0.3) + ((100 - \\text{Adverse Event Rate: }${formData.adverseEventRate}) * 0.2) + ((100 - \\text{Severe Adverse Event Rate: }${formData.severeAdverseEventRate}) * 0.2) + ((10 - \\text{Drug Interaction Index: }${formData.drugInteractionIndex}) * 10 * 0.1) + (\\text{Self Administration Safety: }${formData.selfAdministrationSafety} * 10 * 0.2)`}
            />
          </div>
          <div>
            <h4 className="font-bold">Prescription Score:</h4>
            <InlineMath
              math={`${formData.prescriptionScore?.toFixed(2)} = (\\text{Efficacy Rate: }${formData.efficacyRate} * 0.25) + (\\text{Adverse Event Rate: }${formData.adverseEventRate} * 0.15) + (\\text{Severe Adverse Event Rate: }${formData.severeAdverseEventRate} * 0.2) + (\\text{Drug Interaction Index: }${formData.drugInteractionIndex} * 5 * 0.1) + ((10 - \\text{Self Administration Safety: }${formData.selfAdministrationSafety}) * 10 * 0.3)`}
            />
          </div>
          <div>
            <h4 className="font-bold">Scheduling Score:</h4>
            <InlineMath
              math={`${formData.schedulingScore?.toFixed(2)} = (\\text{Abuse Potential Score: }${formData.abusePotentialScore} * 0.3) + (\\text{Withdrawal Severity Index: }${formData.withdrawalSeverityIndex} * 10 * 0.2) + (\\text{Overdose Risk Factor: }${formData.overdoseRiskFactor} * 10 * 0.2) + (\\text{Misuse Probability: }${formData.misuseProbability} * 0.2) + ((10 - \\text{Therapeutic Index: }${formData.therapeuticIndex}) * 10 * 0.1)`}
            />
          </div>
          <div>
            <h4 className="font-bold">Access Restriction Benefit Score:</h4>
            <InlineMath
              math={`${formData.accessRestrictionBenefitScore?.toFixed(2)} = ((100 - \\text{Access Benefit Ratio: }${formData.accessBenefitRatio}) * 0.6) + ((100 - \\text{Societal Cost Index: }${formData.societalCostIndex}) * 0.4)`}
            />
          </div>
          <div>
            <h4 className="font-bold">Final Classification Score:</h4>
            <InlineMath
              math={`${formData.finalClassificationScore?.toFixed(2)} = (\\text{OTC Score: }${formData.otcScore?.toFixed(2)} * 0.3) + (\\text{Prescription Score: }${formData.prescriptionScore?.toFixed(2)} * 0.3) + (\\text{Scheduling Score: }${formData.schedulingScore?.toFixed(2)} * 0.2) + (\\text{Access Restriction Benefit Score: }${formData.accessRestrictionBenefitScore?.toFixed(2)} * 0.2)`}
            />
          </div>
        </div>
      </div>
