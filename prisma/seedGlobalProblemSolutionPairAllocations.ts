...
export async function seedGlobalProblemSolutionPairAllocations(testUser: User) {
  ...
  console.log(
    `Seeding GlobalProblem Solution Pair Allocations for ${pairs.length} pairs`
  )
  let count = 0
    ...
    const existing = existingAllocations.find((allocation) => {
      return (
        allocation.thisGlobalProblemSolutionId ===
          pair.thisGlobalProblemSolution.id &&
        allocation.thatGlobalProblemSolutionId ===
          pair.thatGlobalProblemSolution.id
      )
    })
    if (existing) {
      console.log(
        `Already allocated ${pair.thisGlobalProblemSolution.name} and ${pair.thatGlobalProblemSolution.name}`
      )
      continue
    }
    const result = await prisma.globalProblemSolutionPairAllocation.create({
      data: {
