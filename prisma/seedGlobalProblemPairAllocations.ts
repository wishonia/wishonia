...
import {aggregateGlobalProblemPairAllocations, getAllRandomGlobalProblemPairs} from "@/lib/globalProblems";
...
export async function seedGlobalProblemPairAllocations(testUser: User) {
    ...
    for (const pair of pairs) {
        ...
        try {
            const existingAllocation = await prisma.globalProblemPairAllocation.findFirst({
                where: {
