...
import {getOwnerRepoFromNpm, scoreContributorsFromPackageJson} from "@/lib/github/npmLibraries";
...
describe("Github API Test", () => {
    ...
    it('should generate the Prisma schema for a GitHub project', async () => {
        const repository = 'wishonia'
        const org = 'wishonia';
        const projects = await octokit.projects.listForRepo({ owner: org, repo: repository });
        if(projects.data.length){
            generatePrismaSchema('GithubProject', projects.data[0], "Github");
            const projectColumns = await octokit.projects.listColumns({ project_id: projects.data[0].id });
            generatePrismaSchema('GithubProjectColumn', projectColumns.data[0], "Github");
            const projectCards = await octokit.projects.listCards({ column_id: projectColumns.data[0].id });
