 ...
 */
import {convertPythonCodeToGithubIssues, convertPythonProjectToNextJS} from "@/lib/agents/python2typescript";
...
describe('convertPythonProjectToNextJS', () => {
    it('should convert Python files to Next.js with TypeScript', async () => {
        ...
        await convertPythonProjectToNextJS(getInputProjectDirectory(), outputDirectory);
    });
    it('should convert Python files to github issues with TypeScript', async () => {
        const outputDirectory = absPathFromRepo('.generated');
        await convertPythonCodeToGithubIssues(getInputProjectDirectory(), outputDirectory+'\\issues');
    });
});
